"""
=======================================================================
  INNOVATION RADAR v9.0 — Global Intelligence Engine
...
def extract_json_safe(text):
    try:
        ...
        # FIX: Mencegah error terpotongnya file dengan mengelabui markdown parser
        # Menggunakan perkalian string literal agar parser tidak mendeteksinya sebagai penutup
        tick3 = '`' * 3
...
def get_coordinates(location_name):
    ...
    headers = {"User-Agent": "InnovationRadarApp/9.0 (research-bot)"}
    try:
        time.sleep(1.5) 
...
def pass_1_validate(api_key, raw_content):
    ...
    res = call_gemini(api_key, raw_content, sys_prompt)
...
{"title": "", "summary": "", "category": [], "innovation_level": "grassroots | semi-formal | institutional", "location": {"country": "", "region": ""}, "process": {"how_it_works": "", "materials_used": [], "step_by_step": []}, "impact": {"problem_solved": "", "scale": "low | medium | high"}, "replicability": {"cost_level": "low | medium | high", "difficulty": "easy | medium | hard"}}"""
    return call_gemini(api_key, raw_content, sys_prompt)
...
def run_discovery_pipeline(api_key, database, max_items=3):
    """Mencari data baru dan menambahkannya ke database, maksimal max_items per sesi."""
    ...
    seed_data = call_gemini(api_key, seed_prompt, seed_sys, use_search=True)
    ...
    for idx, raw_text in enumerate(raw_descriptions):
        if success_count >= max_items:
            log.info(f"Reached maximum limit of {max_items} items for this run. Stopping loop.")
            break
        ...
        if not base_data or not base_data.get("title"): continue
            
        title_hash = hashlib.md5(base_data["title"].encode('utf-8')).hexdigest()
...
def generate_intelligence_report(api_key, database):
    """Membaca data.json dan menambahkan resume baru ke dalam resume.json."""
    if not database:
...
def convert_report_to_markdown(report_data):
    ...
    md += "\n---\n*Report auto-generated by Innovation Radar AI Framework.*\n"
    return md
...
def main():
    ...
    try:
        ...
        # === 1. Tentukan Job yang Berjalan berdasarkan RUN_TYPE ===
        if run_type == "force_data":
            ...
            # Mode Otomatis (Cek Histori Waktu)
            log.info("⏳ TRIGGER: Auto Schedule Mode. Checking Timestamps...")
            
            # Data.json: Crawl per 2 hari
            if now - last_data_time >= timedelta(days=2):
                do_data = True
                log.info("-> Data schedule triggered (>= 2 days).")
            else:
                log.info(f"-> Data schedule skipped. Last run: {last_data_time.strftime('%Y-%m-%d')}.")
            
            # Resume.json: Generate per 90 hari (~3 bulan)
            if now - last_resume_time >= timedelta(days=90):
                do_resume = True
                log.info("-> Resume schedule triggered (>= 90 days).")
            else:
                log.info(f"-> Resume schedule skipped. Last run: {last_resume_time.strftime('%Y-%m-%d')}.")

        # === 2. Eksekusi Pipeline Data ===
        if do_data:
            log.info("--- 🟢 STARTING DATA PIPELINE ---")
            found = run_discovery_pipeline(api_key, db, max_items=3)
            if found > 0:
